...
{
    public static class Applicationlib
    {
        static string ConnectionString = WebConfigurationManager.ConnectionStrings["SqlConnectionString"].ConnectionString.ToString();

        public static SqlDBSetting sqlDBSetting = new SqlDBSetting(BCE.Data.DBServerType.SQL2000, ConnectionString, 10500);
        ...
        public static long ApprovalLevelCount(string userID)
        {
            ...
            approvalCount = BCE.Data.Convert.ToInt64(sqlDBSetting.ExecuteScalar("select ApprovalCount from SCH_Level where upper(LevelCode)=?", jobTitle));
        ...
        public static bool ApprovalCurrentProcess(string userID, long docKey, out long approvalLevel)
        {
            ...
            try
            {
                ...
                if (dtApproval.Rows.Count > 0)
                {
                    ...
                    {
                        approvalLevel = BCE.Data.Convert.ToInt64(dtApproval.Rows[0][1]);
                        validF = true;
                    }
                }
            }
        ...
        public static long ApprovalTotalProcess(string userID)
        {
            ...
            try
            {
                ...
                totalProcess = BCE.Data.Convert.ToInt64(sqlDBSetting.ExecuteScalar(SqlQuery, userID));
            }
            catch (Exception ex)
            {
                totalProcess = -1;

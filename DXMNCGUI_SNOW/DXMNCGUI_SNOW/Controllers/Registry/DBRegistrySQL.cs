...
{
    internal class DBRegistrySQL : DBRegistry
    {
        public override long IncOne(int regID, long startValue)
        {
            using (SqlDBSetting dbSetting = this.myDBSetting.StartTransaction())
            {
                DataTable dataTable = dbSetting.GetDataTable("SELECT * FROM SCH_Registry WITH (UPDLOCK, ROWLOCK) WHERE RegID=" + (object)regID, false, new object[0]);
                long num1;
                ...
                dbSetting.SimpleSaveDataTable(dataTable, "SELECT * FROM SCH_Registry");
                dbSetting.Commit();
                return num1;
            }
        }
        ...
        public override object GetValue(int regID, object startValue)
        {
            if (!this.myUseCache)
            {
                return this.myDBSetting.ExecuteScalar("SELECT RegValue FROM SCH_Registry WHERE RegID=" + (object)regID, new object[0]) ?? startValue;
            }
            else
            {
                if (this.myCacheDBRegistryTable == null)
                    this.myCacheDBRegistryTable = this.myDBSetting.GetDataTable("SELECT RegID, RegValue FROM SCH_Registry ORDER BY RegID", true, new object[0]);
        ...
        public override void SetValue(int regID, object regValue)
        {
            using (SqlDBSetting dbSetting = this.myDBSetting.StartTransaction())
            {
                DataTable dataTable = dbSetting.GetDataTable("SELECT * FROM SCH_Registry WHERE RegID=" + (object)regID, false, new object[0]);
                if (dataTable.Rows.Count > 0)
                {

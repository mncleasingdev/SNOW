...
namespace DXMNCGUI_SNOW.Controllers.Registry
{
    public class DBRegistry
    {
        ...
        public static DBRegistry CreateDBRegistry(SqlDBSetting dbSetting)
        {
        ...
        {
            return BCE.Data.Convert.ToSByte(this.GetValue(reg.ID, reg.DefaultValue));
        }

        public short GetInt16(IRegistryID reg)
        {
            return BCE.Data.Convert.ToInt16(this.GetValue(reg.ID, reg.DefaultValue));
        }

        public int GetInt32(IRegistryID reg)
        {
            return BCE.Data.Convert.ToInt32(this.GetValue(reg.ID, reg.DefaultValue));
        }

        public long GetInt64(IRegistryID reg)
        {
            return BCE.Data.Convert.ToInt64(this.GetValue(reg.ID, reg.DefaultValue));
        }

        public ulong GetUInt64(IRegistryID reg)
        {
            return BCE.Data.Convert.ToUInt64(this.GetValue(reg.ID, reg.DefaultValue));
        }

        public Decimal GetDecimal(IRegistryID reg)
        {
            return BCE.Data.Convert.ToDecimal(this.GetValue(reg.ID, reg.DefaultValue));
        }

        public bool GetBoolean(IRegistryID reg)
        {
            return BCE.Data.Convert.ToBoolean(this.GetValue(reg.ID, reg.DefaultValue));
        }

...
{
    public class TicketRequestSql : TicketRequestDB
    {
        public override DataTable LoadBrowseTable(bool bViewAll, string userID)
        {
            ...
            if (!bViewAll)
            {
                myDBSetting.LoadDataTable(myBrowseTable, "SELECT *, RequestList.CreatedUserID + ' - ' + Users.FULLNAME as IDFULLNAME FROM dbo.RequestList INNER JOIN Users on RequestList.CreatedUserID = Users.NIK WHERE RequestList.CreatedUserID=? ORDER BY TicketReqDate Desc", true, userID);
            }
            else
            {
                myDBSetting.LoadDataTable(myBrowseTable, "SELECT *, RequestList.CreatedUserID + ' - ' + Users.FULLNAME as IDFULLNAME FROM dbo.RequestList INNER JOIN Users on RequestList.CreatedUserID = Users.NIK ORDER BY TicketReqDate Desc", true);
            }
        ...
        public override DataTable LoadBrowseTableDetail(string sTicketNo)
        {
            ...
            myDBSetting.LoadDataTable(myBrowseTableDetail, "SELECT RequestListDetail.TicketNo, RequestListDetail.Action, RequestListDetail.ActionDateTime, RequestListDetail.ActionByID + ' - ' + Users.FULLNAME as IDFULLNAME, RequestList.Reason FROM dbo.RequestListDetail INNER JOIN dbo.RequestList ON RequestListDetail.TicketNo = RequestList.TicketNo INNER JOIN Users ON RequestListDetail.ActionByID = Users.NIK WHERE RequestListDetail.TicketNo=? ORDER BY RequestListDetail.ActionDateTime Desc", true, sTicketNo);
        ...
        protected override DataSet LoadData(long headerid)
        {
            ...
            string sSQLHeader = "SELECT * FROM dbo.RequestList WHERE DocKey=@DocKey";
        ...
        public override void Delete(long headerid)
        {
            ...
            try
            {
                dbSetting.ExecuteNonQuery("DELETE FROM dbo.RequestList WHERE DocKey=?", (object)headerid);
                dbSetting.Commit();
        ...
        protected override void SaveData(TicketRequestNewEntity Ticket, DataSet ds, string strDocName, SaveAction saveaction, DataTable dtCopyApp)
        {
            SqlDBSetting dbSetting = this.myDBSetting.StartTransaction();
            ...
            try
            {
                ...
                if (saveaction == SaveAction.Save)
                {
                    ...
                    DataRow[] myrowDocNo = dbSetting.GetDataTable("select * from DocNoFormat", false, "").Select("DocType='RQ'", "", DataViewRowState.CurrentRows);
                    if (myrowDocNo != null)
                    {
                ...
                {
                    dbSetting.SimpleSaveDataTable(ds.Tables["Header"], "SELECT * FROM dbo.RequestList");
        ...
        protected override void SaveRequestDetailData(TicketRequestNewEntity Ticket, SaveAction saveaction, string aID)
        {
            DateTime Mydate = myDBSetting.GetServerTime();
            SqlConnection myconn = new SqlConnection(myDBSetting.ConnectionString);
            SqlCommand sqlCommand = new SqlCommand("INSERT INTO [dbo].[RequestListDetail] (TicketNo, Action, ActionDateTime, ActionByID) VALUES (@TicketNo, @Action, @ActionDateTime, @ActionByID)");
